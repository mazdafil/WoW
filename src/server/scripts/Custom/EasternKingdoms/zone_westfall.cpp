...
public:
    ...
    struct npc_horatio_lane_42308AI : public ScriptedAI
    {
        ...
        ObjectGuid    m_playerGUID, m_vernaGUID, m_invest_vernaGUID, m_horseGUID, m_invest_horseGUID;
    ...
    struct npc_ripsnarl_sentinel_hillAI : public ScriptedAI
    {
        ...
        void DoWork()
        {
            ...
            case 0:
                ...
                {
                    if (!HasPlayerSeenVideo1(player->GetGUIDLow()))
                    {
                        if (player->HasAura(SPELL_DETECT_QUEST_INVIS_3) && player->GetQuestStatus(QUEST_IN_DEFENCE_OF_WESTFALL) == QUEST_STATUS_INCOMPLETE)
                            ...
                            {
                                _playerList1.push_back(player->GetGUIDLow());
                                _phase = 1;
                            }
                    }
                    else if (!HasPlayerSeenVideo2(player->GetGUIDLow()))
                    {
                        ...
                        {
                            _phase = 20;
                            _playerList2.push_back(player->GetGUIDLow());
                        }
                    }
                }
...
public:
    ...
    struct npc_vision_of_the_past_VehicleAI : public VehicleAI
    {
		...
		ObjectGuid      m_mageGUID;
		ObjectGuid      m_vanessaGUID;
            ...
            if (Player* player = who->ToPlayer())
                if (player->GetQuestStatus(QUEST_VISION_OF_THE_PAST) == QUEST_STATUS_INCOMPLETE)
                    if (me->GetPosition().GetExactDist2d(player) < 10.0f  && abs(me->GetPositionZ() - player->GetPositionZ()) < 2.0f)
                        if (CanPlayerTalk(player->GetGUIDLow()))
                        {
                            Talk(1, player);
                            pList[player->GetGUIDLow()] = 30;
                        }
        }
...
public:
    ...
    struct npc_hope_saldean_42749AI : public ScriptedAI
    {
        ...
        EventMap m_events;
        ObjectGuid   m_playerGUID;
        ObjectGuid   m_gryanGUID;
        ObjectGuid   m_glubtokGUID;
        ObjectGuid   m_ripsnarlGUID;
        uint32   m_point;
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            while (uint32 eventId = m_events.ExecuteEvent())
            {
                ...
                case EVENT_START_WALK:
                {
                    if (Player* player = ObjectAccessor::GetPlayer(*me, m_playerGUID))
                    {
                        player->KilledMonsterCredit(119704);
