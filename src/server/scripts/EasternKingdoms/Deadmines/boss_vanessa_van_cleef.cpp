...
public:
    ...
    {
        boss_vanessa_van_cleefAI(Creature* creature) : BossAI(creature, BOSS_VANESSA_VANCLEEF) 
        { 
        ...
        void Reset()
        {
            ...
            _Reset();
                ...
                if (m_instance->GetData(BOSS_VANESSA_VANCLEEF) == DONE)
                    me->SetVisible(true);
                else
                    me->SetVisible(false);
        ...
        void UpdateAI(uint32 diff)
        {
			...
			events.Update(diff);
                ...
                if (me->HealthBelowPct(50) && m_phase == 0)
                {
                    m_phase = 1;
                    Talk(SAY_ADDS);
...
class npc_vanessa_vancleef_a_note_from_vanessa : public CreatureScript
{
    public:
        ...
        bool OnGossipHello(Player* pPlayer, Creature* pCreature)
        {
            ...
            if (!pInstance)
                return true;
            if (pInstance->GetBossState(BOSS_CAPTAIN_COOKIE) != DONE)
                return true;
            if (pInstance->GetData(BOSS_VANESSA_VANCLEEF) != NOT_STARTED)
                return true;

            pCreature->SummonCreature(NPC_VANESSA_SITTING, vanessaPos);
            pInstance->SetData(BOSS_VANESSA_VANCLEEF, IN_PROGRESS);
...
class npc_vanessa_vancleef_vanessa_sitting : public CreatureScript
{
    public:
        ...
        {
            npc_vanessa_vancleef_vanessa_sittingAI(Creature* pCreature) : BossAI(pCreature, BOSS_VANESSA_VANCLEEF)
            {
            ...
            void UpdateAI(const uint32 diff)
            {
                ...
                if (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_INTRO_7:
                            if (pInstance)
                                pInstance->SetData(BOSS_VANESSA_VANCLEEF, DONE);
...
class npc_vanessa_vancleef_defias_enforcer : public CreatureScript
{
    public:
        ...
        {
            npc_vanessa_vancleef_defias_enforcerAI(Creature* pCreature) : BossAI(pCreature, BOSS_VANESSA_VANCLEEF)
            {
                me->setActive(true);
                pInstance = me->GetInstanceScript();
            }
