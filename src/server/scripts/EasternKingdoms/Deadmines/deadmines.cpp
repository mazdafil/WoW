...
public:
    ...
    struct go_factory_door_13965_AI : public GameObjectAI
    {
        ...
        {
            if (m_instance)
                if (Creature* glubtok = ObjectAccessor::GetCreature(*go, m_instance->GetGuidData(DATA_GLUBTOK)))
                    if (GameObject* door = ObjectAccessor::GetGameObject(*go, m_instance->GetGuidData(DATA_FACTORY_DOOR)))
                        if (GameObject* lever = ObjectAccessor::GetGameObject(*go, m_instance->GetGuidData(DATA_FACTORY_DOOR_LEVER)))
                            if (glubtok->IsAlive())
                            {
...
public:
    ...
    struct go_mastroom_door_16400_AI : public GameObjectAI
    {
        ...
        void OnStateChanged(uint32 state, Unit* unit)
        {
            ...
            if (m_instance)
            {
                if (Creature* olaf = ObjectAccessor::GetCreature(*go, m_instance->GetGuidData(DATA_LUMBERING_OAF)))
                    if (olaf->IsAlive())
                        mustBeClosed = true;
                if (Creature* helix = ObjectAccessor::GetCreature(*go, m_instance->GetGuidData(DATA_HELIX_GEARBREAKER)))
                    if (helix->IsAlive())
                        mustBeClosed = true;
                if (GameObject* door = ObjectAccessor::GetGameObject(*go, m_instance->GetGuidData(DATA_MASTROOM_DOOR)))
                    if (mustBeClosed)
                        door->SetGoState(GO_STATE_READY);
                    else
                        door->SetGoState(GO_STATE_ACTIVE); // open the door  GO_STATE_READY  GO_STATE_ACTIVE  GO_STATE_ACTIVE_ALTERNATIVE
...
public:
    ...
    struct go_foundry_door_16399_AI : public GameObjectAI
    {
        ...
        {
            if (m_instance)
                if (Creature* foo = ObjectAccessor::GetCreature(*go, m_instance->GetGuidData(DATA_FOO_REAPER_5000)))
                    if (GameObject* door = ObjectAccessor::GetGameObject(*go, m_instance->GetGuidData(DATA_FOUNDRY_DOOR)))
                        if (foo->IsAlive())
                            door->SetGoState(GO_STATE_READY);
                        else
                            door->SetGoState(GO_STATE_ACTIVE); // open the door  GO_STATE_READY  GO_STATE_ACTIVE  GO_STATE_ACTIVE_ALTERNATIVE
...
public:
    ...
    struct go_ironclad_lever_101833_AI : public GameObjectAI
    {
        ...
        {
            if (_instance > 0 && state == 2)
                if (GameObject* door = ObjectAccessor::GetGameObject(*go, _instance->GetGuidData(DATA_IRONCLAD_DOOR)))
                    door->SetGoState(GO_STATE_ACTIVE); // open the door  GO_STATE_READY  GO_STATE_ACTIVE  GO_STATE_ACTIVE_ALTERNATIVE
        }
    };
...
public:
    ...
    struct go_ironclad_door_16397_AI : public GameObjectAI
    {
        ...
        {
            if (m_instance)
                if (GameObject* door = ObjectAccessor::GetGameObject(*go, m_instance->GetGuidData(DATA_IRONCLAD_DOOR)))
                    door->SetGoState(GO_STATE_READY);
        }
    };
...
public:
    ...
    struct go_defias_cannon_16398_AI : public GameObjectAI
    {
        ...
        void OnStateChanged(uint32 state, Unit* unit) // the door is allways closed until destroyed..
        {
            if (m_instance)
                if (state = 2)
                    if (GameObject* cannon = ObjectAccessor::GetGameObject(*go, m_instance->GetGuidData(DATA_DEFIAS_CANNON))) // m_instance->GetGameObject(DefiasCannonGUID))
                        if (GameObject* door = ObjectAccessor::GetGameObject(*go, m_instance->GetGuidData(DATA_IRONCLAD_DOOR)))
                        {
        ...
        void DoWork()
        {
            ...
            case 0:
            {
                ...
                if (Creature* bunny = me->FindNearestCreature(NPC_TARGET_BUNNY, 125.0f))
                    if (Player* player = me->FindNearestPlayer(125.0f, true))
                    {
                        ...
                        if (worker)
                        {
                            ...
                            m_WorkCounter = 0;
                            worker->HandleEmoteCommand(EMOTE_STATE_WORK);
                            m_phase = 1;
                            m_timer = 5000;
                            return;
                        }
                ...
                if (m_WorkCounter < 2)
                {
                    m_WorkCounter++;
                    if (oger)
                        oger->HandleEmoteCommand(EMOTE_ONESHOT_NONE);
                    if (kobold)
                    {
                    ...
                    if (defias)
                        defias->HandleEmoteCommand(EMOTE_ONESHOT_NONE);
                }
                break;
            }
            case 1:
