    ...
    if (!IsHighestExclusiveAura(aura))
    {
        if (!aura->GetSpellInfo()->IsAffectingArea(GetMap()->GetDifficultyID()))
        {
            ...
            if (caster && caster->GetTypeId() == TYPEID_PLAYER)
                Spell::SendCastResult(caster->ToPlayer(), aura->GetSpellInfo(), aura->GetCastGUID(), SPELL_FAILED_AURA_BOUNCED);
        }
...
void Unit::RemoveNotOwnSingleTargetAuras(uint32 newPhase, bool phaseid) 
{
    ...
    {
        Aura* aura = *iter;
        if (aura->GetUnitOwner() != this && !aura->GetUnitOwner()->IsInPhase(newPhase) && aura->GetUnitOwner()->GetEntry() != 42693)
        {
            aura->Remove();
            iter = scAuras.begin();
        }
